...
export async function populateProducts(
	...
	const [brands, categories, discounts, session] =
		await Promise.all([
			BrandCache.get(),
			CategoryCache.get(),
			DiscountCache.get(),
	...
	const populatedProducts = products.map((product) => {
		...
		const applicableDiscounts = discounts.filter(
			(discount) => discount.id in product.discounts
		)
		const discount = applicableDiscounts.reduce(
			(prev, next) =>
				(prev =
					(Number(next.expires) > Date.now() &&
...
async function getProducts(searchParams: TSearchParams) {
	...
	const [brandList, categoryList, discountList] =
		await Promise.all([
			BrandCache.get(),
			CategoryCache.get(),
			DiscountCache.get(),
	...
	if (query.brand)
		query.brand =
			brandList
				?.find((brand) => brand.id.toString() === query.brand)
				?.id.toString() || ""
	if (query.category)
		query.category =
			categoryList
				?.find((cat) => cat.id.toString() === query.category)
				?.id.toString() || ""
