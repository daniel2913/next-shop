...
import { getUserVotes, updateVoteAction } from "@/actions/rating"
import {
	getProductsByIdsAction,
	getProductsPageAction,
} from "@/actions/product"
import { getSavedAction, toggleSavedAction } from "@/actions/saved"
...
export interface ProductsSlice {
	...
	updateVote: (
		id: number,
		vote: number
	) => Promise<ServerErrorType | { rating: number; voters: number }>
...
const useProductStore = createWithEqualityFn<ProductsSlice>()((set, get) => ({
	...
	loadProducts: async (page: number | undefined, query: URLSearchParams) => {
		...
		const res = await queryProducts(
			query,
			Object.keys(get().products).length,
			page
		)
		if (!res) return false
	...
	reloadSingle: async (id) => {
		...
		if (idx === -1)
			return { error: "Something is broken", title: "Client Error" }
