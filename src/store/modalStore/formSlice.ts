...
export interface ModalFormSlice{
    form: {
        formFieldProps: {[i in keyof ModalFormSlice['form']['formFieldValues']]:props}
        formFieldValues: {[i:string]:string|FileList|File}
        accepted: boolean
		action:string
        setFormFieldValues: (newValues: {[i:string]:string|FileList|File}) => void
        setFormFieldProps: (props: {[i in keyof ModalFormSlice['form']['formFieldValues']]:props}) => void
        reset: () => void
    }
}
