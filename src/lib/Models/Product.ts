...
const ProductInsertValidation = z.object({
	...
	images: z
		...
		.optional()
		.transform((files) =>
			files ? handleImages(toArray(files), "products") : undefined
		)
		.pipe(z.array(validations.imageName).default(["template.jpg"])),
})
...
const ProductPgreTable = shop.table("products", config, (table) => {
	return {
